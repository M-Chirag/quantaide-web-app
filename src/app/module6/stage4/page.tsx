...
export default function Stage4() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const parsedDataF: Record<string, { score: number; feedback: string }> = JSON.parse(parsedData);
    const items: any[] = Object.entries(parsedDataF).map(([key, value]: [string, any]) => ({
      number: key,
      score: value.score,
              ...
              <div className='flex flex-col w-[69%] max-md:ml-0 max-md:w-full'>
                <div className='flex flex-col mt-9 text-black max-md:mt-10 max-md:max-w-full'>
                  <div className='flex gap-5 self-start'>
                    ...
                    <div className='flex flex-col grow shrink-0 justify-end px-5 basis-0 w-fit'>
                      <div className='text-xs'>Question Formulation</div>
                      {!assessmentResult && <div className='mt-1 mb-5 text-xl font-bold'>
                        Let&apos;s create a survey
                      </div>}
                    </div>
                  </div>
                            ...
                            <div className='flex gap-1 items-center text-3xl font-bold tracking-wider '>
                                      ...
                                      {[...Array(5)].map((_, index) => (
                                        <Star
                                          key={index}
                                          className={`my-auto ${
                                            index < assessmentResult[0]["score"]? scoreMappings[assessmentResult[0].score]?.color : 'text-gray-300'
                                          }`}
                                        />
                                      ))}
                                    </div>
